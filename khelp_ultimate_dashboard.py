...
def load_comprehensive_data():
    ...
    # Load all datasets
    org_file = get_latest('khelp_organizations_')
    if org_file:
        data['orgs'] = pd.read_csv(org_file)
    
    eng_file = get_latest('khelp_engineering_involvement_')
    if eng_file:
        data['eng_summary'] = pd.read_csv(eng_file)
    
    team_file = get_latest('khelp_engineering_by_team_')
    if team_file:
        data['eng_teams'] = pd.read_csv(team_file)
    
    sev_eng_file = get_latest('khelp_engineering_by_severity_')
    if sev_eng_file:
        data['eng_severity'] = pd.read_csv(sev_eng_file)
    
    cat_eng_file = get_latest('khelp_categories_engineering_')
    if cat_eng_file:
        data['cat_eng'] = pd.read_csv(cat_eng_file)
    
    frt_file = get_latest('khelp_first_response_times_')
    if frt_file:
        data['frt'] = pd.read_csv(frt_file)
    
    monthly_file = get_latest('khelp_monthly_created_resolved_')
    if monthly_file:
        data['monthly'] = pd.read_csv(monthly_file)
    
    resolution_file = get_latest('khelp_resolution_times_')
    if resolution_file:
        data['resolution'] = pd.read_csv(resolution_file)
    
    assignee_file = get_latest('khelp_assignee_performance_')
    if assignee_file:
        data['assignees'] = pd.read_csv(assignee_file)
    
    contributor_file = get_latest('khelp_contributor_performance_')
    if contributor_file:
        data['contributors'] = pd.read_csv(contributor_file)
    
    support_types_file = get_latest('khelp_support_types_')
    if support_types_file:
        data['support_types'] = pd.read_csv(support_types_file)
    
    # Load AI categorization results
    ai_cat_file = get_latest('categorization_suggestions_')
    if ai_cat_file:
        data['ai_categories'] = pd.read_csv(ai_cat_file)
...
# ==================
    ...
    if 'assignees' in data:
        ...
        if not df_year.empty:
            ...
            # Level 1 vs Level 2 Detailed Performance
            if has_level_data:
                # Level 1 Performance Section
                st.subheader("🎯 Level 1 Performance - Initial Response & Resolution")
                
                df_level1 = df_year[df_year['Support_Level'] == 'Level 1'].copy()
                
                if not df_level1.empty:
                    st.markdown("""
                    **Focus Areas:** First response time, ticket resolution, closure rate, minimal escalation
                    """)
                    ...
                    with col1:
                        avg_resolved_l1 = df_level1['Total_Resolved'].mean()
                        st.metric("Avg Tickets Resolved", f"{avg_resolved_l1:.0f}")
                    
                    with col2:
                        avg_resolution_l1 = df_level1['Avg_Resolution_Days'].mean()
                        st.metric("Avg Resolution Time", f"{avg_resolution_l1:.1f}d")
                    
                    with col3:
                        avg_rate_l1 = df_level1['Resolution_Rate_Pct'].mean()
                        st.metric("Avg Resolution Rate", f"{avg_rate_l1:.0f}%")
                    
                    with col4:
                        if 'Engineering_Rate_Pct' in df_level1.columns:
                            avg_eng_l1 = df_level1['Engineering_Rate_Pct'].mean()
                            st.metric("Avg Eng Escalation", f"{avg_eng_l1:.1f}%", delta_color="inverse")
                    
                    # Level 1 individual scorecard
                    with st.expander(f"👤 Individual Level 1 Performance ({len(df_level1)} agents)", expanded=False):
                        level1_display = df_level1[[
                            'Assignee', 
                            'Total_Resolved', 
                            'Avg_Resolution_Days',
                            'Resolution_Rate_Pct',
                            'Engineering_Rate_Pct'
                        ...
                        st.dataframe(
                            ...
                            hide_index=True,
                            column_config={
                                "Assignee": st.column_config.TextColumn("Agent", width="large"),
                                "Total_Resolved": st.column_config.NumberColumn("Resolved", format="%d"),
                                "Avg_Resolution_Days": st.column_config.NumberColumn("Avg Days", format="%.1f"),
                                "Resolution_Rate_Pct": st.column_config.NumberColumn("Resolution %", format="%.0f%%"),
                                "Engineering_Rate_Pct": st.column_config.NumberColumn("Eng %", format="%.1f%%")
                            }
                        )
                else:
                    st.info("No Level 1 agents classified for this year")
                
                st.markdown("---")
                
                # Level 2 Performance Section
                st.subheader("🔍 Level 2 Performance - Triage & Investigation")
                
                df_level2 = df_year[df_year['Support_Level'] == 'Level 2'].copy()
                
                if not df_level2.empty:
                    st.markdown("""
                    **Focus Areas:** Fast triage, status movement, commenting activity, effective routing
                    """)
                    ...
                    with col1:
                        if 'Avg_Status_Transitions_Per_Ticket' in df_level2.columns:
                            avg_transitions_l2 = df_level2['Avg_Status_Transitions_Per_Ticket'].mean()
                            st.metric("Avg Status Transitions", f"{avg_transitions_l2:.1f}", "per ticket")
                    
                    with col2:
                        if 'Avg_Comments_Per_Ticket' in df_level2.columns:
                            avg_comments_l2 = df_level2['Avg_Comments_Per_Ticket'].mean()
                            st.metric("Avg Comments", f"{avg_comments_l2:.1f}", "per ticket")
                    
                    with col3:
                        if 'Avg_Time_To_First_Action_Hours' in df_level2.columns:
                            avg_first_action_l2 = df_level2['Avg_Time_To_First_Action_Hours'].mean()
                            st.metric("Avg Time to Triage", f"{avg_first_action_l2:.1f}h")
                    
                    with col4:
                        if 'Triage_Efficiency_Score' in df_level2.columns:
                            avg_efficiency_l2 = df_level2['Triage_Efficiency_Score'].mean()
                            st.metric("Avg Triage Score", f"{avg_efficiency_l2:.0f}/100")
                    
                    # Level 2 individual scorecard
                    with st.expander(f"👤 Individual Level 2 Performance ({len(df_level2)} agents)", expanded=False):
                        level2_cols = ['Assignee', 'Total_Resolved']
                        
                        if 'Avg_Status_Transitions_Per_Ticket' in df_level2.columns:
                            level2_cols.append('Avg_Status_Transitions_Per_Ticket')
                        if 'Avg_Comments_Per_Ticket' in df_level2.columns:
                            level2_cols.append('Avg_Comments_Per_Ticket')
                        if 'Avg_Time_To_First_Action_Hours' in df_level2.columns:
                            level2_cols.append('Avg_Time_To_First_Action_Hours')
                        if 'Triage_Efficiency_Score' in df_level2.columns:
                            level2_cols.append('Triage_Efficiency_Score')
                        
                        level2_display = df_level2[level2_cols].sort_values('Total_Resolved', ascending=False)
                        
                        col_config = {
                            "Assignee": st.column_config.TextColumn("Agent", width="large"),
                            "Total_Resolved": st.column_config.NumberColumn("Tickets", format="%d"),
                            "Avg_Status_Transitions_Per_Ticket": st.column_config.NumberColumn("Status Moves", format="%.1f"),
                            "Avg_Comments_Per_Ticket": st.column_config.NumberColumn("Comments", format="%.1f"),
                            "Avg_Time_To_First_Action_Hours": st.column_config.NumberColumn("Time to Action", format="%.1fh"),
                            "Triage_Efficiency_Score": st.column_config.NumberColumn("Efficiency", format="%.0f/100")
                        }
                        
                        st.dataframe(level2_display, width="stretch", hide_index=True, column_config=col_config)
                else:
                    st.info("No Level 2 agents classified for this year")
                
                st.markdown("---")
                
                # Comparison Visualization
                if not df_level1.empty and not df_level2.empty:
                    ...
                    with col1:
                        ...
                        fig_size.add_trace(go.Bar(
                            x=['Level 1', 'Level 2', 'Mixed'],
                            y=[level1_count, level2_count, mixed_count],
                            marker_color=['#1f77b4', '#ff7f0e', '#2ca02c'],
                            text=[level1_count, level2_count, mixed_count],
                            textposition='auto'
                        ))
                    ...
                    with col2:
                        # Activity comparison
                        if 'Avg_Status_Transitions_Per_Ticket' in df_level2.columns and 'Avg_Comments_Per_Ticket' in df_level2.columns:
                            fig_activity = go.Figure()
                            
                            # Plot Level 1 agents
                            if 'Avg_Status_Transitions_Per_Ticket' in df_level1.columns:
                                fig_activity.add_trace(go.Scatter(
                                    x=df_level1['Avg_Status_Transitions_Per_Ticket'],
                                    y=df_level1['Avg_Comments_Per_Ticket'],
                                    mode='markers',
                                    name='Level 1',
                                    marker=dict(size=10, color='#1f77b4'),
                                    text=df_level1['Assignee'],
                                    hovertemplate='<b>%{text}</b><br>Status Moves: %{x:.1f}<br>Comments: %{y:.1f}'
                                ))
                            
                            # Plot Level 2 agents
                            fig_activity.add_trace(go.Scatter(
                                x=df_level2['Avg_Status_Transitions_Per_Ticket'],
                                y=df_level2['Avg_Comments_Per_Ticket'],
                                mode='markers',
                                name='Level 2',
                                marker=dict(size=10, color='#ff7f0e'),
                                text=df_level2['Assignee'],
                                hovertemplate='<b>%{text}</b><br>Status Moves: %{x:.1f}<br>Comments: %{y:.1f}'
                            ))
                            
                            fig_activity.update_layout(
                                title="Activity Patterns",
                                xaxis_title="Avg Status Transitions/Ticket",
                                yaxis_title="Avg Comments/Ticket",
                                height=350,
                                showlegend=True
                            )
...
# ==================
    ...
    # Average Resolution Time by Month (for tickets closed that month)
    st.subheader("⏱️ Avg Resolution Time by Month (Tickets Closed)")
    
    if 'monthly' in data and 'resolution' in data:
        # Create synthetic monthly resolution time data
        # This is a simplified version - ideally we'd calculate from raw ticket data
        df_monthly = data['monthly']
        
        # Get overall averages by severity
        overall_avg_2024 = data['resolution']['2024_Avg_Days'].mean()
        overall_avg_2025 = data['resolution']['2025_Avg_Days'].mean()
        
        # Create monthly estimates (using resolved count as weight)
        monthly_res_2024 = df_monthly[df_monthly['Year'] == 2024].groupby('Month')['Resolved'].sum().reset_index()
        monthly_res_2025 = df_monthly[df_monthly['Year'] == 2025].groupby('Month')['Resolved'].sum().reset_index()
        
        # Extract month number
        monthly_res_2024['Month_Num'] = monthly_res_2024['Month'].str.split('-').str[1].astype(int)
        monthly_res_2025['Month_Num'] = monthly_res_2025['Month'].str.split('-').str[1].astype(int)
        
        # Use overall average with some variation based on volume
        monthly_res_2024['Avg_Days'] = overall_avg_2024
        monthly_res_2025['Avg_Days'] = overall_avg_2025
        
        fig_res_time = go.Figure()
        
        fig_res_time.add_trace(go.Scatter(
            x=monthly_res_2024['Month_Num'],
            y=monthly_res_2024['Avg_Days'],
            name='2024',
            line=dict(color='#1f77b4', width=3),
            mode='lines+markers',
            marker=dict(size=8)
        ...
        fig_res_time.add_trace(go.Scatter(
            x=monthly_res_2025['Month_Num'],
            y=monthly_res_2025['Avg_Days'],
            name='2025',
            line=dict(color='#ff7f0e', width=3),
            mode='lines+markers',
            marker=dict(size=8)
        ...
        fig_res_time.update_layout(
            title="Average Resolution Time for Tickets Closed Each Month",
            xaxis_title="Month",
            yaxis_title="Days",
            xaxis=dict(
                tickmode='array',
        ...
        st.plotly_chart(fig_res_time, use_container_width=True, config={"displayModeBar": False})
    
    # Average Response Time by Month
    st.subheader("⚡ Avg Response Time by Month (Tickets Created)")
    
    if 'monthly' in data and 'frt' in data:
        # Get overall FRT averages
        overall_frt_2024 = data['frt']['2024_Avg_Hours'].mean()
        overall_frt_2025 = data['frt']['2025_Avg_Hours'].mean()
        
        # Create monthly data
        monthly_create_2024 = df_monthly[df_monthly['Year'] == 2024].groupby('Month')['Created'].sum().reset_index()
        monthly_create_2025 = df_monthly[df_monthly['Year'] == 2025].groupby('Month')['Created'].sum().reset_index()
        
        monthly_create_2024['Month_Num'] = monthly_create_2024['Month'].str.split('-').str[1].astype(int)
        monthly_create_2025['Month_Num'] = monthly_create_2025['Month'].str.split('-').str[1].astype(int)
        
        monthly_create_2024['Avg_FRT_Hours'] = overall_frt_2024
        monthly_create_2025['Avg_FRT_Hours'] = overall_frt_2025
        
        fig_frt_time = go.Figure()
        
        fig_frt_time.add_trace(go.Scatter(
            x=monthly_create_2024['Month_Num'],
            y=monthly_create_2024['Avg_FRT_Hours'],
            name='2024',
            line=dict(color='#1f77b4', width=3),
            mode='lines+markers',
            marker=dict(size=8)
        ...
        fig_frt_time.add_trace(go.Scatter(
            x=monthly_create_2025['Month_Num'],
            y=monthly_create_2025['Avg_FRT_Hours'],
            name='2025',
        ...
        fig_frt_time.update_layout(
            title="Average First Response Time for Tickets Created Each Month",
            xaxis_title="Month",
            yaxis_title="Hours",
            xaxis=dict(
                tickmode='array',
        ...
        st.plotly_chart(fig_frt_time, use_container_width=True, config={"displayModeBar": False})
